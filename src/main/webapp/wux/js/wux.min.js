...
s!=e&&(r+=s+" ")}return r.trim()}function I(t,e){if(!t)return e
if(!e)return t
...
break
case t.WInputType.CheckBox:if(!this.checkboxStyle){var T=Math.round(.8*parseInt(this.root.css("font-size")));(isNaN(T)||T<16)&&(T=16),this.checkboxStyle="height:"+T+"px;"}var W='<input type="checkbox" name="'+p.id+'" id="'+p.id+'" class="'+this.inputClass+'" '
this.checkboxStyle.length>2&&(W+='style="'+this.checkboxStyle+'" '),p.readonly&&(W+="readonly "),0==p.enabled&&(W+="disabled "),W+="/>",p.element=$(W)
break
case t.WInputType.Radio:p.component&&p.component.mount(f)
break
case t.WInputType.Select:var I=""
if(p.attributes?(I+='<select name="'+p.id+'" id="'+p.id+'" class="'+this.inputClass+'" '+p.attributes,(p.readonly||0==p.enabled)&&(I+=" disabled"),I+=">"):(I+='<select name="'+p.id+'" id="'+p.id+'" class="'+this.inputClass+'"',(p.readonly||0==p.enabled)&&(I+="disabled"),I+=">"),p.options&&p.options.length>0)for(var R=0;R<p.options.length;R++){var D=p.options[R]
I+="string"==typeof D?"<option>"+D+"</option>":'<option value="'+D.id+'">'+D.text+"</option>"}I+="</select>",p.element=$(I)
break
...
M.append(F)
for(var P=0,H=this.footer;P<H.length;P++){var B=H[P]
"string"==typeof B&&B.length>0?F.append($(B)):B instanceof t.WComponent?B.mount(F):F.append(B)}}for(var V=this,j=0,X=this.dpids;j<X.length;j++){var G=X[j]
!function(n){var r=t.WUtil.getDate(V.minValues,n)
...
c>=19000101&&u>c&&(t.showWarning(t.RES.ERR_DATE),$(r.target).datepicker("setDate",l))}}}})}(G)}"object"==typeof this.state&&this.updateView(),this.root.find("input").keypress(function(t){if(13==t.which){for(var n=$(t.target).attr("id"),r=e.next(n);r&&!e.isFocusable(r);)r=e.next(r.id)
